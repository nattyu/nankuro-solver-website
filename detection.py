...
def process_filtered_boxes(
    ...
    """
    フィルタ済みボックスに対して文字認識のみ行い、
    (x1, y1, x2, y2, recognized_char, conf, class_name) を返す。
    ★ 計測用に全体時間・件数も出力する。
    """
    detected_characters = []
    ...
    print(
        f"[OCR] process_filtered_boxes: total={elapsed:.3f}s "
        f"(avg={avg*1000:.1f}ms/box, "
        f"kanji={count_kanji}, number={count_number}, black={count_black}, "
        f"total_boxes={total})"
    )
